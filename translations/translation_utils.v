...
Class Translation := { tsl_id : ident -> ident ;
                       tsl_tm : tsl_context -> term -> TemplateMonad term ;
                       tsl_ty : tsl_context -> term -> TemplateMonad term ;
                       tsl_ind : tsl_context -> kername -> kername -> minductive_decl
                            -> TemplateMonad (tsl_table * list minductive_decl)
                     }.
...
Definition tTranslate {tsl : Translation} (ΣE : tsl_context) (id : ident)
  ...
  | Some (IndRef (mkInd kn n)) =>
      ...
      Edecls <- tmEval lazy (tsl_ind ΣE kn kn' d) ;; Edecls <- Edecls;; (*join*)
      let (E, decls) := (Edecls : tsl_table * list minductive_decl) in
  ...
  | Some (ConstRef kn) => 
    ...
    | DefinitionEntry {| definition_entry_type := A;
      ...
      t' <- tmEval lazy (tsl_tm ΣE t) ;; t' <- t' ;;
      tmMkDefinition id' (t' : term) ;;
...
Definition tImplement {tsl : Translation} (ΣE : tsl_context)
  ...
  : TemplateMonad (tsl_context) :=
  tA <- tmQuote A ;;
  tA' <- tmEval lazy (tsl_ty ΣE tA) ;; tA' <- tA' ;;
  id' <- tmEval all (tsl_id id) ;;
